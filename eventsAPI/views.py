...
def get_events_in_country(request, country):
    ...
    db_data = Events.objects.filter(event_country= country)
    
    
    if db_data:
        x = Events.objects.filter(event_country= country).values()
        y = list(x)
        time_created = y[0]
        ...
        if current_time - time_hour >= 6 :
            ...
            Events.objects.filter(event_country= country).delete()
            for event in events:
                Events.objects.create(event_name=event['title'],event_id=event['id'],event_country=event['country'],event_date=event['start'],event_lat=event["location"][0],event_lon=event["location"][1],desc= event["description"])
                
            events = Events.objects.filter(event_country= country).values()
...
def get_event_wehather(request,eventid):
     ...
     event = list(Events.objects.filter(event_id = eventid).values())[0]
     lat = event['event_lat']
     lon = event['event_lon']
